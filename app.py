...
st.markdown(
    ...
    <ul>
        ...
        <li>Historical and cultural sites</li>
...
with overview_tab:
    ...
    st.caption(
        "Start here for the big picture, then use the tabs above to zoom in — "
        "from provinces, down to districts, down to individual languages."
    )
    ...
    add_pakistan_boundary(overview_map)
    folium.LayerControl(collapsed=False).add_to(overview_map)
    st_folium(overview_map, width=1200, height=550, key="overview_map")

    st.divider()

    m1, m2, m3, m4, m5 = st.columns(5)
    with m1:
        st.metric("Languages Tracked", len(full_languages))
    with m2:
        st.metric("Total Speakers", f"{int(full_languages['Speakers'].sum()):,}")
    with m3:
        st.metric("Provinces", full_languages["Province"].nunique())
    with m4:
        districts_geojson_preview = load_districts_geojson()
        district_count = len(districts_geojson_preview["features"]) if districts_geojson_preview else 0
        st.metric("Districts Mapped", district_count)
    with m5:
        st.metric("Cultural Sites", len(poets))

    st.caption(
        "Click through the tabs above — each one narrows the focus a little further, "
        "from the whole country down to a single language's exact numbers."
    )
